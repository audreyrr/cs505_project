...
Usage:
 ...
 python src/plot_olmo2_results.py

 python src/plot_olmo2_results.py --results \
   results/results_olmo2_1b_vllm.json \
   results/results_olmo2_7b_vllm.json \
   results/results_olmo2_13b_vllm.json

 python src/plot_olmo2_results.py --output figures/olmo2_accuracy.png
"""
...
def load_avg_accuracy(path: Path):
    ...
    # Keep only positive steps for log-scale plotting
    pairs = sorted((s, a) for s, a in zip(steps, accs) if s > 0)
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="figures/olmo2_new_log_accuracy.png",
    ...
    ax.set_xlabel("Log Training Step", fontsize=12)
    ax.set_ylabel("Average Accuracy (all tasks)", fontsize=12)
    ax.set_title("OLMo-2 — ICL Accuracy vs Log Training Step", fontsize=13)
